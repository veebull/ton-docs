...
| Send all received tokens together with the contract balance | `mode` = 128, `flag` = 0 | `send_raw_message(msg, 128)` | `balance`: 0, `send`: 100 + 50 + 20 - 3 = 167
...
| Send all received tokens together with the contract balance and destroy smart-contract | `mode` = 128, `flag` = 32 | `send_raw_message(msg, 160)` | `balance`: 0, `send`: 100 + 50 + 20 - 3 = 167
| Send all received tokens together with the contract balance and destroy smart-contract, if there was an error in processing the action - bounce the message in addition to rolling back the transaction. `IMPORTANT: Avoid this behaviour because the refunds will go to an already deleted contract.`  | `mode` = 128, `flag` = 32 + 16 | `send_raw_message(msg, 176)` | `balance`: 0 + `147 (bounced)`, `send`: 100 + 50 + 20 - 3 = `bounce` message with 147
